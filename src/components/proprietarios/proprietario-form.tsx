...
export function ProprietarioForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!aceite) { toast('Confirme que o proprietário foi informado sobre o tratamento de dados (LGPD)', 'error'); return }
    setLoading(true)
    const res = await fetch('/api/admin/criar-proprietario', {
      method: 'POST',
    ...
    toast(`Proprietário cadastrado! Compartilhe com ele: e-mail ${form.email} e a senha definida.`, 'success')
    ...
    <Card>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <Input id="cpf" label="CPF/CNPJ" value={form.cpf} onChange={set('cpf')} placeholder="000.000.000-00" />
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Compartilhe o e-mail e esta senha diretamente com o proprietário (WhatsApp, etc.) para ele acessar o portal.
            </p>
          </div>
          <label className="flex items-start gap-2 text-xs text-gray-600">
            ...
            />
            <span>
              Confirmo que tenho autorização para cadastrar estes dados e que o proprietário foi informado
              sobre o tratamento de dados pessoais conforme a Política de Privacidade e a LGPD.
            </span>
          </label>
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => router.back()}>Cancelar</Button>
            <Button type="submit" loading={loading}>Cadastrar Proprietário</Button>
          </div>
        </form>
      </CardContent>
    </Card>
