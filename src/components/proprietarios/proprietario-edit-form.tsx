...
export function ProprietarioEditForm({ proprietario }: { proprietario: Profile }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    const res = await fetch('/api/admin/editar-proprietario', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: proprietario.id, ...form }),
    })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <fieldset disabled={!podeEditar('proprietarios')} className="space-y-6">
      <div className="space-y-4">
        ...
        <Input id="cpf" label="CPF/CNPJ" value={form.cpf} onChange={set('cpf')} />
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Só preencha se quiser trocar a senha do proprietário. Compartilhe a nova senha com ele diretamente.
            </p>
          </div>
        </CardContent>
      </Card>
